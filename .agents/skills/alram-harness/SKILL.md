---
name: alram-harness
description: Use when working in this project with the Harness workflow, including exploration, planning phases, step files, execution, and review.
---
...
2. **자기완결성** — 각 step 파일은 독립된 에이전트 세션(Codex 또는 Claude Code)에서 실행된다. "이전 대화에서 논의한 바와 같이" 같은 외부 참조는 금지한다. 필요한 정보는 전부 파일 안에 적는다.
...
- `project`: 프로젝트명 (AGENTS.md 또는 CLAUDE.md 참조).
...
| 전이 | 기록되는 필드 | 기록 주체 |
|------|-------------|----------|
| → `started` | `started_at` | execute.py (자동) |
...
2. 아키텍처 체크리스트를 확인한다:
   ...
   - AGENTS.md/CLAUDE.md CRITICAL 규칙을 위반하지 않았는가?
...
execute.py는 git repo를 필수로 요구하며, npm scripts가 정의된 package.json과 설치된 의존성이 있어야 Stop hook이 정상 동작한다.
...
```bash
python3 scripts/execute.py {task-name}                  # auto runner로 순차 실행
python3 scripts/execute.py {task-name} --runner codex   # Codex로 실행
python3 scripts/execute.py {task-name} --runner claude  # Claude Code로 실행
python3 scripts/execute.py {task-name} --push           # 실행 후 push
```

runner 선택 우선순위:
1. CLI `--runner codex|claude`
2. 환경변수 `HARNESS_AGENT_RUNNER=codex|claude`
3. auto 감지 (Codex 환경이면 Codex 우선, 일반 터미널에서는 기존 호환성을 위해 Claude Code 우선)
