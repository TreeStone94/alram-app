...
### ADR-008: 날씨 캐시 TTL = 24시간

**결정**: 기상청 API 응답을 AsyncStorage에 저장, 24시간 TTL.

**이유**:
- 단기예보는 하루 8회 발표 (3시간 간격) — 24시간 이내 데이터는 당일 날씨에 유효
- 알람 해제 시 (아침) 전날 밤 또는 당일 새벽 캐시 → 충분히 신선
- 일 10,000회 API 호출 한도 보호
